...
def _day(parse: Callable[[str], XsInstant | None], value: str) -> int:
    instant = parse(value)
...
class TestDaysFromCivil:
    ...
    def test_matches_datetime(self, date: datetime.date) -> None:
        assert days_from_civil(date.year, date.month, date.day) == (date - _EPOCH).days
    ...
    def test_years_beyond_datetime_continue_the_cycle(self) -> None:
        ...
        assert days_from_civil(12024, 1, 1) == days_from_civil(2024, 1, 1) + 25 * 146097
        assert days_from_civil(-2376, 1, 1) == days_from_civil(2024, 1, 1) - 11 * 146097
        assert days_from_civil(0, 1, 1) == days_from_civil(400, 1, 1) - 146097
...
class TestBceYears:
    ...
    def test_1_bce_is_the_year_before_1_ce_for_every_type(self) -> None:
        # 1 BCE is a leap year in the proleptic Gregorian calendar.
    ...
    def test_bce_leap_years_follow_the_proleptic_gregorian_cycle(self, value: str) -> None:
    ...
    def test_lexical_year_zero_is_rejected(self, parse: Callable[[str], XsInstant | None], value: str) -> None:
...
def test_parsers_cover_the_wide_year_types() -> None:
    assert set(PARSERS) == {"date", "dateTime", "gYear", "gYearMonth"}
