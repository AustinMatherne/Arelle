...
from arelle.oim.csv.metadata.model import XbrlCsvDocumentInfo, XbrlCsvEffectiveMetadata, XbrlCsvTable
...
def _tables(**tables: XbrlCsvTable) -> XbrlCsvEffectiveMetadata:
    return XbrlCsvEffectiveMetadata(document_info=_DOCUMENT_INFO, tables=MappingProxyType(tables))
...
def _template(
    ...
    return TCTemplateConstraints(constraints=MappingProxyType(constraints), column_order=column_order)
...
class TestCells:
    def test_valid_rows_produce_no_errors(self) -> None:
        tc = _tc(t=_template(n=TCValueConstraint("xs:integer"), s=TCValueConstraint("xs:string", optional=True)))
        rows = [["n", "s", "other"], ["1", "a", ""], ["2", "", "x"], ["3", "#empty", ""]]
    ...
    def test_invalid_value_is_reported_with_its_location(self) -> None:
        ...
        assert (error.table_id, error.url, error.row, error.column) == ("t", "t.csv", 3, "n")
        assert str(error) == "table 't' row 3 column 'n': value 'x' is not valid for the xs:integer constraint, url: t.csv"

    @pytest.mark.parametrize(
        "constraint, value, expected_code",
        [
            (TCValueConstraint("xs:date", time_zone=True), "2024-01-01", "tcre:missingTimeZone"),
            (TCValueConstraint("xs:date", time_zone=False), "2024-01-01Z", "tcre:unexpectedTimeZone"),
            (TCValueConstraint("period", period_type="year"), "2024Q1", "tcre:invalidPeriodType"),
            (TCValueConstraint("xs:duration", duration_type="dayTime"), "P1Y", "tcre:invalidDurationType"),
    ...
    def test_short_rows_are_treated_as_empty_cells(self) -> None:
        tc = _tc(t=_template(n=TCValueConstraint("xs:integer", optional=True), s=TCValueConstraint("xs:string")))
        assert _codes(_run(_SINGLE_TABLE, tc, {"t.csv": [["s", "n"], ["a"]]})) == []
        assert _codes(_run(_SINGLE_TABLE, tc, {"t.csv": [["n", "s"], ["1"]]})) == ["tcre:missingValue"]

    def test_repeated_header_uses_the_first_column(self) -> None:
        tc = _tc(t=_template(n=TCValueConstraint("xs:integer")))
        assert _codes(_run(_SINGLE_TABLE, tc, {"t.csv": [["n", "n"], ["1", "x"]]})) == []


class TestHeader:
    def test_required_column_missing_from_header(self) -> None:
        tc = _tc(t=_template(n=TCValueConstraint("xs:integer"), o=TCValueConstraint("xs:integer", optional=True)))
        (error,) = _run(_SINGLE_TABLE, tc, {"t.csv": [["other"]]})
        assert (error.code, error.column, error.row) == ("tcre:missingColumn", "n", None)
    ...
    def test_column_order_requires_all_columns_once(self) -> None:
        tc = _tc(t=_template(column_order=("a", "b", "c"), a=TCValueConstraint("xs:string", optional=True)))
    ...
    def test_column_order_is_enforced(self) -> None:
        ...
        assert _codes(_run(_SINGLE_TABLE, tc, {"t.csv": [["c", "a", "b"]]})) == ["tcre:invalidColumnOrder"]
...
class TestTables:
    def test_template_defaults_to_table_id_and_tables_share_templates(self) -> None:
        ...
        tc = _tc(t=_template(n=TCValueConstraint("xs:integer")), unconstrained=TCTemplateConstraints())
        files = {"t.csv": [["n"], ["x"]], "u.csv": [["n"], ["y"]], "v.csv": [["n"], ["z"]]}
        opened: list[str] = []
        errors = _run(csv_metadata, tc, files, opened)
        assert [(error.table_id, error.url) for error in errors] == [("t", "t.csv"), ("u", "u.csv")]
        assert opened == ["t.csv", "u.csv"], "tables of templates without report checks are not read"
    ...
    def test_progress_is_reported_per_table(self) -> None:
        messages: list[str] = []
        tc = _tc(t=_template(n=TCValueConstraint("xs:integer")))
        validator = TCReportValidator(_SINGLE_TABLE, tc, lambda table_id, table: iter([["n"], ["1"]]), messages.append)
