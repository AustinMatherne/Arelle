...
from arelle.oim._tc.report.cell import UnknownSpecialValue, effective_value, row_has_value
...
from arelle.oim.const import XBRLCE_UNKNOWN_SPECIAL_VALUE
...
"""Returns the rows of a table's CSV file, header first, or None when there is no file to read."""

_PROGRESS_ROW_INTERVAL = 10000
_NIL_LITERAL = "#nil"
...
class TCReportValidator:
    ...
    def _compile_templates(self) -> Mapping[str, _Template]:
        ...
        return {
            template_id: _Template(
                columns=tuple(
                    _ConstrainedColumn(name, constraint, ValueConstraintValidator(constraint, namespaces))
    ...
    def _validate_table(
        ...
        present_columns = [
            (column, column_indexes[column.name]) for column in template.columns if column.name in column_indexes
        ]
        for row_number, row in enumerate(rows, start=2):
            if row_number % _PROGRESS_ROW_INTERVAL == 0:
                self._report_progress(_("Validating table constraints of table {} row {}").format(table_id, row_number))
    ...
    def _validate_header(
        self, table_id: str, table: XbrlCsvTable, template: _Template, column_indexes: Mapping[str, int]
    ) -> Generator[TCReportValidationError, None, None]:
        missing_columns = [
            column.name
        ...
        if template.column_order is not None:
            missing_columns.extend(
                name for name in template.column_order if name not in column_indexes and name not in missing_columns
            )
        ...
        if template.column_order is None:
            return
        expected_order = [name for name in template.column_order if name in column_indexes]
        if sorted(expected_order, key=column_indexes.__getitem__) != expected_order:
            yield TCReportValidationError(
                _("columns must appear in the order {}").format(", ".join(template.column_order)),
    ...
    @staticmethod
    def _validate_cell(column: _ConstrainedColumn, literal: str) -> _CellViolation | None:
        constraint = column.constraint
        if literal == _NIL_LITERAL and not constraint.nillable:
            return _CellViolation(TCRE_INVALID_VALUE, _("#nil is not permitted, the column is not nillable"))
        try:
            value = effective_value(literal)
        except UnknownSpecialValue:
            return _CellViolation(XBRLCE_UNKNOWN_SPECIAL_VALUE, _("unknown special value {!r}").format(literal))
        if value is None:
            if constraint.optional:
                return None
            return _CellViolation(TCRE_MISSING_VALUE, _("the column is required and has no value"))
...
def _describe_violation(code: str, value: str, constraint: TCValueConstraint) -> str:
    ...
    if code == TCRE_INVALID_PERIOD_TYPE:
        return _("value {!r} is not a period of type {}").format(value, constraint.period_type)
    if code == TCRE_INVALID_DURATION_TYPE:
        return _("value {!r} is not a duration of type {}").format(value, constraint.duration_type)
    return _("value {!r} is not valid for the {} constraint").format(value, constraint.type)
