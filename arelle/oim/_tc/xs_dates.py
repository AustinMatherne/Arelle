...
XML Schema date, dateTime, gYear and gYearMonth values with any year, parsed into
comparable instants with proleptic Gregorian day arithmetic because datetime only
covers years 1 to 9999.
"""
...
_TZ = r"(?P<tz>Z|[+-](?:(?:0[0-9]|1[0-3]):[0-5][0-9]|14:00))?"
...
def days_from_civil(year: int, month: int, day: int) -> int:
    """Days from 1970-01-01 to the given proleptic Gregorian date, for any integer year."""
    # Years start in March so that leap days fall at the end of the year.
    if month <= 2:
        year -= 1
    era = year // 400
...
def _tz_offset_seconds(tz: str | None) -> int:
    ...
    return sign * (int(tz[1:3]) * 3600 + int(tz[4:6]) * 60)
...
PARSERS: Mapping[str, Callable[[str], XsInstant | None]] = MappingProxyType(
    {
        "date": parse_date,
