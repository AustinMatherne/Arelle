...
def validateTableConstraints(modelXbrl: ModelXbrl) -> None:
    """Runs xBRL-CSV Table Constraints validation for an xBRL-CSV report.

    Report validation only runs when the metadata is valid, since constraints defined
    by inconsistent metadata have no reliable meaning.
    """
    csvContext = modelXbrl.xbrlCsvLoadingContext
    if csvContext is None or csvContext.tc_metadata is None:
        return
    hasMetadataErrors = False
    tcMetadataValidator = TCMetadataValidator(csvContext.metadata, csvContext.tc_metadata)
    ...
    tcReportValidator = TCReportValidator(
        ...
        lambda tableId, table: _tableConstraintsTableRows(modelXbrl, csvContext, tableId, table),
...
def _tableConstraintsTableRows(
    modelXbrl: ModelXbrl, csvContext: XbrlCsvLoadingContext, tableId: str, table: XbrlCsvTable
) -> Iterator[list[str]] | None:
    """Opens the CSV file of a table for table constraints validation.

    Workbook tables are not validated. A missing file is reported when the report
    was not loaded, since the loader reports it otherwise.
    """
    metadataPath = csvContext.metadata_path or ""
    tableUrl = table.url
    isWorkbookTable = "#" in tableUrl or tableUrl.endswith((".xlsx", ".xls"))
    if isWorkbookTable or (metadataPath.endswith((".xlsx", ".xls")) and ".csv" not in tableUrl):
        return None
    tablePath = os.path.normpath(os.path.join(os.path.dirname(metadataPath), tableUrl))
